...
class GeneralEncoder(nn.Module):
    ...
    def set_mask(self, mask):
        ...
        self._reduced_indices = mask.bool().nonzero(as_tuple=False).squeeze(-1)
        self._full_lat_dim = self.lat_dim
        self.lat_dim = int(mask.sum().item())

    def forward(self, x, params = None, controls = None):
        """
        x: [batch_size, input_dim]
        Returns: mu [batch_size, lat_dim] (reduced if mask set) — single tensor in deterministic mode
        ...
        mu, logvar = torch.split(self.net(x), self._full_lat_dim if self.mask_set else self.lat_dim, dim=1)

        if self.mask_set:
            return mu[:, self._reduced_indices]
...
class LatentODEFunc(nn.Module):
    ...
    def set_mask(self, mask, mask_controls=None, mask_parameters=None):
        ...
        mask: [lat_state_mu_dim] mask for the latent state (boolean tensor or tensor with values 0 or 1)
        After calling this, lat_state_dim and lat_state_mu_dim are updated to reduced dimensions.
        The forward method will expand reduced inputs to full dim for the network, then reduce back.
        '''
        self.mask_set.set_(torch.tensor(True, device=mask.device))
        if self.lat_ode_type == 'variance_dynamic':
            ...
            self.mask.set_(torch.tensor(mask.clone().detach(), device=mask.device, dtype=torch.float32))
        # store reduced indices and original dimensions, then overwrite dimensions
        self._reduced_state_indices = mask.bool().nonzero(as_tuple=False).squeeze(-1)
        self._full_lat_state_dim = self.lat_state_dim
        self._full_lat_state_mu_dim = self.lat_state_mu_dim
        self._full_lat_control_dim = self.lat_control_dim
        self._full_lat_parameter_dim = self.lat_parameter_dim
        self.lat_state_mu_dim = int(mask.sum().item())
        self.lat_state_dim = self.lat_state_mu_dim if not self.lat_ode_type == 'variance_dynamic' else 2 * self.lat_state_mu_dim
        # controls mask
        if mask_controls is not None:
            self._reduced_control_indices = mask_controls.bool().nonzero(as_tuple=False).squeeze(-1)
            self.lat_control_dim = int(mask_controls.sum().item())
        else:
            self._reduced_control_indices = None
        # parameters mask
        if mask_parameters is not None:
            self._reduced_parameter_indices = mask_parameters.bool().nonzero(as_tuple=False).squeeze(-1)
            self.lat_parameter_dim = int(mask_parameters.sum().item())
        else:
            self._reduced_parameter_indices = None
    ...
    def _expand_to_full(self, reduced, indices, full_dim):
        """Expand a reduced-dim tensor back to full dimension, filling zeros."""
        _full = torch.zeros(reduced.shape[0], full_dim, device=reduced.device, dtype=reduced.dtype)
        _full[:, indices] = reduced
        return _full
    ...
    def forward(self, lat_states, lat_parameters = None, lat_controls=None, 
        ...
        """
        lat_state: [batch_size, lat_state_dim] (reduced if mask set)
        lat_parameters: [batch_size, lat_parameter_dim] (reduced if mask set)
        lat_controls: [batch_size, lat_control_dim] (reduced if mask set)
        """
        # In deterministic mode, expand reduced inputs to full dimension for network call
        if self.mask_set:
            lat_states = self._expand_to_full(lat_states, self._reduced_state_indices, self._full_lat_state_dim)
            if lat_controls is not None and self._reduced_control_indices is not None:
                lat_controls = self._expand_to_full(lat_controls, self._reduced_control_indices, self._full_lat_control_dim)
            if lat_parameters is not None and self._reduced_parameter_indices is not None:
                lat_parameters = self._expand_to_full(lat_parameters, self._reduced_parameter_indices, self._full_lat_parameter_dim)
        ...
        if not self.linear:
            if not self.lat_ode_type == 'variance_dynamic' or (self.lat_ode_type == 'variance_dynamic' and self.lat_state_mu_independent is False):
                ...
                _states_mu = lat_states[:, :self._full_lat_state_mu_dim if self.mask_set else self.lat_state_mu_dim]
                if self.include_controls and self.include_parameters:
            ...
            if self.include_parameters:
                ...
                if self.include_controls:
                    B_from_param = B_from_param.view(-1, self._full_lat_state_dim if self.mask_set else self.lat_state_dim, self._full_lat_control_dim if self.mask_set and self._reduced_control_indices is not None else self.lat_control_dim)
        ...
        # In deterministic mode, reduce output back to populated dimensions
        if self.mask_set:
            lat_states_dot = lat_states_dot[:, self._reduced_state_indices]
...
class Decoder(nn.Module):
    ...
    def set_mask(self, mask_states, mask_controls=None, mask_parameters=None):
        ...
        After calling this, lat_state_mu_dim etc. are updated to reduced dimensions.
        The forward method will expand reduced inputs to full dim before the network call.
        '''
        self.mask_set = True
        self._reduced_state_indices = mask_states.bool().nonzero(as_tuple=False).squeeze(-1)
        self.lat_state_mu_dim = int(mask_states.sum().item())
        if mask_controls is not None and self.include_controls:
            self._reduced_control_indices = mask_controls.bool().nonzero(as_tuple=False).squeeze(-1)
            self.lat_control_dim = int(mask_controls.sum().item())
        else:
            self._reduced_control_indices = None
        if mask_parameters is not None and self.include_parameters:
            self._reduced_parameter_indices = mask_parameters.bool().nonzero(as_tuple=False).squeeze(-1)
            self.lat_parameter_dim = int(mask_parameters.sum().item())
        else:
            self._reduced_parameter_indices = None
    ...
    def forward(self, 
        ...
        """
        lat_state: [batch_size, lat_state_mu_dim] (reduced if mask set)
        lat_parameters: [batch_size, lat_parameter_dim] (reduced if mask set)
        lat_controls: [batch_size, lat_control_dim] (reduced if mask set)
        ...
        # In deterministic mode, expand reduced inputs to full dimension for network call
        if self.mask_set:
            lat_state = self._expand_to_full(lat_state, self._reduced_state_indices, self._full_lat_state_mu_dim)
            if lat_controls is not None and self._reduced_control_indices is not None:
                lat_controls = self._expand_to_full(lat_controls, self._reduced_control_indices, self._full_lat_control_dim)
            if lat_parameters is not None and self._reduced_parameter_indices is not None:
                lat_parameters = self._expand_to_full(lat_parameters, self._reduced_parameter_indices, self._full_lat_parameter_dim)
        ...
        if not self.linear:
            ...
            if not self.include_parameters:
                ...
                C, constant = torch.split(_C, [self.out_dim * self._full_lat_state_mu_dim, self.out_dim], dim=1)
                C = C.view(-1, self.out_dim, self._full_lat_state_mu_dim)
                ...
                if self.include_controls:
                    ...
                    D = D.view(-1, self.out_dim, self._full_lat_control_dim)
