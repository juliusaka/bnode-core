...
def _build_reduced_input_indices(state_idx, full_state_dim,
                                 ctrl_idx, full_ctrl_dim, include_controls,
                                 param_idx, full_param_dim, include_parameters,
                                 feedthrough_dim=0):
    """Build column indices for a concatenated input [states, params, controls, feedthrough]."""
    indices = state_idx
    offset = full_state_dim
    if include_parameters:
        p_idx = param_idx if param_idx is not None else torch.arange(full_param_dim, device=state_idx.device)
        indices = torch.cat([indices, p_idx + offset])
        offset += full_param_dim
    if include_controls:
        c_idx = ctrl_idx if ctrl_idx is not None else torch.arange(full_ctrl_dim, device=state_idx.device)
        indices = torch.cat([indices, c_idx + offset])
        offset += full_ctrl_dim
    if feedthrough_dim > 0:
        ft_idx = torch.arange(feedthrough_dim, device=state_idx.device)
        indices = torch.cat([indices, ft_idx + offset])
    return indices


def _trim_sequential_net(net, input_indices, output_indices=None):
    """Trim a Sequential network's first-layer columns and optionally last-layer rows."""
    with torch.no_grad():
        first_layer = net[0]
        first_layer.weight = nn.Parameter(first_layer.weight[:, input_indices])
        if output_indices is not None:
            last_layer = net[-1]
            last_layer.weight = nn.Parameter(last_layer.weight[output_indices])
            last_layer.bias = nn.Parameter(last_layer.bias[output_indices])
...
class GeneralEncoder(nn.Module):
    ...
    def set_mask(self, mask):
        already_trimmed = self.mask_set.item()
        self.mask_set.set_(torch.tensor(True, device=mask.device))
        self.mask.set_(mask.clone().detach().to(device=mask.device, dtype=torch.float32))
        reduced_indices = mask.bool().nonzero(as_tuple=False).squeeze(-1)
        full_lat_dim = self.lat_dim
        self.lat_dim = int(mask.sum().item())
        if not already_trimmed:
            # Trim last layer to output only mu for populated dims
            last_layer = self.net[-1]
            # Original output: [mu_0..mu_full, logvar_0..logvar_full]
            # Keep only the mu rows at reduced_indices
            with torch.no_grad():
                last_layer.weight = nn.Parameter(last_layer.weight[reduced_indices])
                last_layer.bias = nn.Parameter(last_layer.bias[reduced_indices])

    def forward(self, x, params = None, controls = None):
        """
        x: [batch_size, input_dim]
        Returns: mu [batch_size, lat_dim] — single tensor in deterministic mode
                 (mu, logvar) tuple in normal mode
        """
        ...
        if self.mask_set:
            ...
            mu, logvar = torch.split(y, self.lat_dim, dim=1)
            return mu, logvar
...
class LatentODEFunc(nn.Module):
    ...
    def set_mask(self, mask, mask_controls=None, mask_parameters=None):
        '''
        Set the mask for the latent state (and optionally controls/parameters), used for deterministic training.
        Trims network first/last layer weights so forward() operates directly on reduced dimensions.
        Idempotent: if mask_set is already True (e.g. after load_state_dict), skips trimming.
        '''
        already_trimmed = self.mask_set.item()
        self.mask_set.set_(torch.tensor(True, device=mask.device))
        if self.lat_ode_type == 'variance_dynamic':
            raise NotImplementedError('masking is not implemented yet for variance_dynamic latent ODEs')
            _mask = torch.zeros(self.lat_state_dim, device=mask.device)
            _mask[:self.lat_state_mu_dim] = mask
            self.mask.set_(_mask.clone().detach().to(device=mask.device, dtype=torch.float32))
        else:
            self.mask.set_(mask.clone().detach().to(device=mask.device, dtype=torch.float32))

        reduced_state_indices = mask.bool().nonzero(as_tuple=False).squeeze(-1)
        full_lat_state_dim = self.lat_state_dim
        full_lat_control_dim = self.lat_control_dim
        full_lat_parameter_dim = self.lat_parameter_dim

        # Compute new reduced dimensions
        new_state_mu_dim = int(mask.sum().item())
        reduced_control_indices = mask_controls.bool().nonzero(as_tuple=False).squeeze(-1) if mask_controls is not None else None
        new_control_dim = int(mask_controls.sum().item()) if mask_controls is not None else self.lat_control_dim
        reduced_param_indices = mask_parameters.bool().nonzero(as_tuple=False).squeeze(-1) if mask_parameters is not None else None
        new_param_dim = int(mask_parameters.sum().item()) if mask_parameters is not None else self.lat_parameter_dim

        # Trim network weights (skip if already trimmed, e.g. after load_state_dict)
        if not already_trimmed:
            if not self.linear:
                self._trim_nonlinear_net_weights(
                    self.net, reduced_state_indices, full_lat_state_dim,
                    reduced_control_indices, full_lat_control_dim,
                    reduced_param_indices, full_lat_parameter_dim)
                if self.lat_state_mu_independent and hasattr(self, 'net_sigma'):
                    raise NotImplementedError('masking is not implemented yet for the net_sigma of variance_dynamic latent ODEs')
                    self._trim_nonlinear_net_weights(
                        self.net_sigma, reduced_state_indices, full_lat_state_dim,
                        reduced_control_indices, full_lat_control_dim,
                        reduced_param_indices, full_lat_parameter_dim)
            # trim linear matrices
            else:
                if not self.include_parameters:
                    self._trim_linear_ssm_weights(
                        reduced_state_indices, full_lat_state_dim,
                        reduced_control_indices, full_lat_control_dim)

        # Update dimensions
        self.lat_state_mu_dim = new_state_mu_dim
        self.lat_state_dim = new_state_mu_dim if not self.lat_ode_type == 'variance_dynamic' else 2 * new_state_mu_dim
        self.lat_control_dim = new_control_dim
        self.lat_parameter_dim = new_param_dim

    def _trim_nonlinear_net_weights(self, net, state_idx, full_state_dim,
                                     ctrl_idx, full_ctrl_dim,
                                     param_idx, full_param_dim):
        """Trim first/last layer of a Sequential net for reduced latent dims."""
        input_indices = _build_reduced_input_indices(
            state_idx, full_state_dim, ctrl_idx, full_ctrl_dim, self.include_controls,
            param_idx, full_param_dim, self.include_parameters)
        # Compute reference output before trimming for verification
        with torch.no_grad():
            device = net[0].weight.device
            probe = torch.randn(1, len(input_indices), device=device)
            probe_full = torch.zeros(1, net[0].in_features, device=device)
            probe_full[0, input_indices] = probe[0]
            expected = net(probe_full)[:, state_idx]
        _trim_sequential_net(net, input_indices, output_indices=state_idx)
        # Verify trimmed network produces the same output
        with torch.no_grad():
            actual = net(probe)
            assert torch.allclose(expected, actual, atol=1e-5), \
                f"Weight trimming changed network output (max diff={(expected - actual).abs().max():.2e})"

    def _trim_linear_ssm_weights(self, state_idx, full_state_dim, ctrl_idx, full_ctrl_dim):
        """Trim linear SSM A/B matrices for reduced latent dims."""
        with torch.no_grad():
            # A: (full_state, full_state) -> (reduced_state, reduced_state)
            self.A.weight = nn.Parameter(self.A.weight[state_idx][:, state_idx])
            if self.include_controls and ctrl_idx is not None:
                # B: (full_state, full_ctrl) -> (reduced_state, reduced_ctrl)
                self.B.weight = nn.Parameter(self.B.weight[state_idx][:, ctrl_idx])
...
class Decoder(nn.Module):
    ...
    def __init__(self, 
                 ...
                 feedthrough_controls_dim: int = 0,
                 ):
        super().__init__()
        self.register_buffer("mask_set", torch.tensor(False))
        self.lat_state_mu_dim = lat_state_mu_dim
        self.lat_control_dim = lat_control_dim
        self.lat_parameter_dim = lat_parameter_dim
        self.feedthrough_controls_dim = feedthrough_controls_dim
        self.include_feedthrough_controls = feedthrough_controls_dim > 0
        ...
        self.feedthrough_normalization = NormalizationLayer1D(feedthrough_controls_dim) if self.include_feedthrough_controls else None
        ...
        # store full dimensions for expand/reduce in deterministic mode
        self._full_lat_state_mu_dim = lat_state_mu_dim
        self._full_lat_control_dim = lat_control_dim
        self._full_lat_parameter_dim = lat_parameter_dim

        if not linear:
            _input_dim = self.lat_state_mu_dim + self.lat_control_dim + self.lat_parameter_dim + self.feedthrough_controls_dim
            modules = [
            ...
            # construct a linear state space model: y = Cx + Du + Ev (where v is feedthrough)
            if not self.include_parameters:
                ...
                if self.include_feedthrough_controls:
                    self.E = nn.Linear(self.feedthrough_controls_dim, self.out_dim, bias=False)
                    initialize_weights_biases(self.E, initialization)
            else:
                ...
                if self.include_feedthrough_controls:
                    self.E_from_param = nn.Linear(self.lat_parameter_dim, self.out_dim * self.feedthrough_controls_dim, bias=True)
                    initialize_weights_biases(self.E_from_param, initialization)

    def set_mask(self, mask_states, mask_controls=None, mask_parameters=None):
        '''
        Set masks for the decoder, used for deterministic mode.
        Trims network first-layer weights so forward() operates directly on reduced dimensions.
        Idempotent: if mask_set is already True (e.g. after load_state_dict), skips trimming.
        '''
        already_trimmed = self.mask_set.item()
        self.mask_set.set_(torch.tensor(True, device=mask_states.device))
        reduced_state_indices = mask_states.bool().nonzero(as_tuple=False).squeeze(-1)
        full_state_dim = self._full_lat_state_mu_dim
        full_ctrl_dim = self._full_lat_control_dim
        full_param_dim = self._full_lat_parameter_dim

        new_state_dim = int(mask_states.sum().item())
        reduced_ctrl_indices = mask_controls.bool().nonzero(as_tuple=False).squeeze(-1) if mask_controls is not None and self.include_controls else None
        new_ctrl_dim = int(mask_controls.sum().item()) if mask_controls is not None and self.include_controls else self.lat_control_dim
        reduced_param_indices = mask_parameters.bool().nonzero(as_tuple=False).squeeze(-1) if mask_parameters is not None and self.include_parameters else None
        new_param_dim = int(mask_parameters.sum().item()) if mask_parameters is not None and self.include_parameters else self.lat_parameter_dim

        # Trim network weights (skip if already trimmed, e.g. after load_state_dict)
        if not already_trimmed:
            if not self.linear:
                input_indices = _build_reduced_input_indices(
                    reduced_state_indices, full_state_dim,
                    reduced_ctrl_indices, full_ctrl_dim, self.include_controls,
                    reduced_param_indices, full_param_dim, self.include_parameters,
                    feedthrough_dim=self.feedthrough_controls_dim)
                # Compute reference output before trimming for verification
                with torch.no_grad():
                    device = self.net[0].weight.device
                    probe = torch.randn(1, len(input_indices), device=device)
                    probe_full = torch.zeros(1, self.net[0].in_features, device=device)
                    probe_full[0, input_indices] = probe[0]
                    expected = self.net(probe_full)
                _trim_sequential_net(self.net, input_indices)
                # Verify trimmed network produces the same output
                with torch.no_grad():
                    actual = self.net(probe)
                    assert torch.allclose(expected, actual, atol=1e-5), \
                        f"Decoder weight trimming changed network output (max diff={(expected - actual).abs().max():.2e})"
            elif not self.include_parameters:
                with torch.no_grad():
                    # @copilot add a verification step to check that the trimmed matrices produce the same output for a given input
                    self.C.weight = nn.Parameter(self.C.weight[:, reduced_state_indices])
                    if self.include_controls and reduced_ctrl_indices is not None:
                        self.D.weight = nn.Parameter(self.D.weight[:, reduced_ctrl_indices])
            else:
                # Parameter-dependent linear: C_from_param outputs (out_dim * full_state + out_dim)
                # Trim the C portion output rows (state dims) and parameter input columns
                with torch.no_grad():
                    W = self.C_and_constant_from_param.weight
                    b = self.C_and_constant_from_param.bias
                    C_out_size = self.out_dim * full_state_dim
                    W_C = W[:C_out_size]  # rows for C matrix
                    W_const = W[C_out_size:]  # rows for constant
                    # Reshape W_C to (out_dim, full_state, in_features), trim state dim, reshape back
                    W_C_3d = W_C.view(self.out_dim, full_state_dim, -1)
                    W_C_trimmed = W_C_3d[:, reduced_state_indices, :].reshape(self.out_dim * new_state_dim, -1)
                    new_W = torch.cat([W_C_trimmed, W_const], dim=0)
                    b_C = b[:C_out_size]
                    b_const = b[C_out_size:]
                    b_C_3d = b_C.view(self.out_dim, full_state_dim)
                    b_C_trimmed = b_C_3d[:, reduced_state_indices].reshape(-1)
                    new_b = torch.cat([b_C_trimmed, b_const])
                    # Trim parameter input columns
                    if reduced_param_indices is not None:
                        new_W = new_W[:, reduced_param_indices]
                    self.C_and_constant_from_param.weight = nn.Parameter(new_W)
                    self.C_and_constant_from_param.bias = nn.Parameter(new_b)
                    if self.include_controls and reduced_ctrl_indices is not None:
                        W_D = self.D_from_param.weight
                        b_D = self.D_from_param.bias
                        W_D_3d = W_D.view(self.out_dim, full_ctrl_dim, -1)
                        new_W_D = W_D_3d[:, reduced_ctrl_indices, :].reshape(self.out_dim * new_ctrl_dim, -1)
                        if reduced_param_indices is not None:
                            new_W_D = new_W_D[:, reduced_param_indices]
                        self.D_from_param.weight = nn.Parameter(new_W_D)
                        b_D_3d = b_D.view(self.out_dim, full_ctrl_dim)
                        self.D_from_param.bias = nn.Parameter(b_D_3d[:, reduced_ctrl_indices].reshape(-1))
                    elif reduced_param_indices is not None:
                        # Only trim parameter input columns for D (no control trimming)
                        if self.include_controls:
                            self.D_from_param.weight = nn.Parameter(self.D_from_param.weight[:, reduced_param_indices])
                    if self.include_feedthrough_controls and reduced_param_indices is not None and hasattr(self, 'E_from_param'):
                        self.E_from_param.weight = nn.Parameter(self.E_from_param.weight[:, reduced_param_indices])

        self.lat_state_mu_dim = new_state_dim
        self.lat_control_dim = new_ctrl_dim
        self.lat_parameter_dim = new_param_dim
    ...
    def forward(self, 
                ...
                feedthrough_controls: torch.Tensor = None,
                ):
        """
        lat_state: [batch_size, lat_state_mu_dim]
        lat_parameters: [batch_size, lat_parameter_dim]
        lat_controls: [batch_size, lat_control_dim]
        feedthrough_controls: [batch_size, feedthrough_controls_dim] - raw (un-encoded) control signals
        """

        # Normalize feedthrough controls if provided
        if feedthrough_controls is not None and self.include_feedthrough_controls:
            feedthrough_controls = self.feedthrough_normalization(feedthrough_controls)
        
        if not self.linear:
            # Build input tensor by concatenating all available components
            components = [lat_state]
            if self.include_parameters:
                components.append(lat_parameters)
            if self.include_controls:
                components.append(lat_controls)
            if self.include_feedthrough_controls:
                components.append(feedthrough_controls)
            
            if len(components) > 1:
                lat_state = torch.cat(components, dim=1)
            else:
                lat_state = components[0]
            ...
            if not self.include_parameters:
                ...
                if self.include_feedthrough_controls:
                    x += self.E(feedthrough_controls)
            else:
                _C = self.C_and_constant_from_param(lat_parameters)
                C, constant = torch.split(_C, [self.out_dim * self.lat_state_mu_dim, self.out_dim], dim=1)
                ...
                if self.include_feedthrough_controls and feedthrough_controls is not None:
                    E = self.E_from_param(lat_parameters)
                    E = E.view(-1, self.out_dim, self.feedthrough_controls_dim)
                    x += torch.matmul(E, feedthrough_controls.unsqueeze(-1)).squeeze(-1)
